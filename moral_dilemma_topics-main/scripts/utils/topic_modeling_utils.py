...
class NMF(TopicModel):
    def __init__(self):
        ...
        # 兼容 sklearn>=1.0:0.23 版 NMF pickle 缺少新版 transform 所需属性
        # (alpha_W/alpha_H/regularization 等),无法直接调用。
        # 改用新版 sklearn NMF 实例包装旧 components_,transform 只依赖
        # H 矩阵与求解参数,结果与旧版一致。
        try:
            old_model.set_params(verbose=0)
            self._model = old_model
        except (AttributeError, ValueError):
            from sklearn.decomposition import NMF as _SklearnNMF
            self._model = _SklearnNMF(n_components=old_model.n_components_,
                                      init="custom")
            self._model.components_ = old_model.components_
            for p in ("solver", "beta_loss", "max_iter", "tol"):
                if hasattr(old_model, p):
                    setattr(self._model, p, getattr(old_model, p))
