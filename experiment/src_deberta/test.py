...
class StanceModel(nn.Module):
    def __init__(self, model_path, num_classes=2, freeze_layers=0, use_mft=False):
        super().__init__()
        self.roberta = AutoModel.from_pretrained(model_path, torch_dtype=torch.float32)
...
def run_experiment(args):
    ...
    print(f"DeBERTa LR: {lr_roberta}, head LR: {lr_head}")
    optimizer = AdamW([
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str,
                        default='F:\\RoBERTa\\RoBERTa-base\\model_debert')
